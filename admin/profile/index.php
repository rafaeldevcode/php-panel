...
<?php getHtml(__DIR__.'/../../partials/header-main.php', ['title' => 'Perfil']) ?>
            ...
            <section class='p-3 bg-cm-light m-3 rounded shadow'>
                <div class='position-relative'>
                    ...
                    <div class='mx-auto position-relative profile-user'>
                        <img class='border border-color-main w-100 position-absolute bottom-0 left-0' src='<?php asset('/assets/images/users/'.$user[0]['avatar']) ?>' alt='<?php echo $user[0]['name'] ?>'/>
                    ...
                    <div class='position-absolute top-0 start-0 m-3 text-color-main'>
                        <p class='p-0 m-0 display-4 fw-bold'><?php echo $user[0]['name'] ?></p>
                    </div>

                    <div class='position-absolute top-0 end-0 m-3'>
                        <span class='text-cm-light bg-cm-<?php echo (is_null($user[0]['status']) || $user[0]['status'] == 'off') ? 'danger' : 'primary' ?> badge badge-sm fw-bold'>
                            <i class='bi bi-circle-fill'></i>
                            <?php echo (is_null($user[0]['status']) || $user[0]['status'] == 'off') ? 'Inativo' : 'Ativo' ?>
                        </span>
                    </div>
                </div>
    ...
    <?php getHtml(__DIR__.'/../../partials/modal-avatars.php', ['user_id' => $user[0]['id'], 'avatar' => $user[0]['avatar']]) ?>
</body>
</html>
